            ...
            for iteration in range(self.max_iterations):
                for i in enumerate(training_data):
                    ...
                    if predicted_label != training_labels[i]:
                        ...
                        # *** YOUR CODE HERE ***
                        numerator = (self.weights[predicted_label] - self.weights[training_labels[i]]) * training_data[i] +1.0
                        denominator = (training_data[i] * training_data[i]) * 2
                        tau = 0
                        if denominator != 0: #dividing by zero == bad
                            tau = min(c, numerator/denominator)

                        #weighting updates
                        for item, value in training_data[i].items():
                            self.weights[training_labels[i]][item] += value*tau
                            self.weights[predicted_label][item] -= value*tau
