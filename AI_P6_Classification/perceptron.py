    ...
    def train(self, training_data, training_labels, validation_data, validation_labels):
        ...
        for iteration in range(self.max_iterations):
            ...
            for i in range(len(training_data)):
                ...
                curr_data = training_data[i]
                scores = util.Counter()

                for label in self.legal_labels:
                    scores[label] = self.weights[label] * curr_data
                next_label_maybe = scores.arg_max()

                #weights adjustment if the prediction of the model was wrong
                if next_label_maybe != curr_label:
                    self.weights[curr_label] += curr_data
                    self.weights[next_label_maybe] -= curr_data
                # END SOLUTION
