...
import game
import util
from distance_calculator import is_int
...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, game_state):
        ...
        if noisy_distance is None:
            ...
            self.beliefs = util.Counter()
            self.beliefs[jail_position] = 1.0
        else:
            all_possible = util.Counter()
            for position in self.legal_positions:
                distance = util.manhattan_distance(position, pacman_position)
                if emission_model[distance] > 0: #important check for valid dis
                    all_possible[position] = self.beliefs[position] * emission_model[distance]
    ...
    def elapse_time(self, game_state):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old_pos in self.legal_positions:
            old_belief = self.beliefs[old_pos]
            if old_belief > 0:
                new_pos_dist = self.get_position_distribution(self.set_ghost_position(game_state, old_pos)) #given
                for new_pos, probability in new_pos_dist.items():
                    new_beliefs[new_pos] += old_belief *probability
...
class ParticleFilter(InferenceModule):
    ...
    def initialize_uniformly(self, game_state):
        ...
        # *** YOUR CODE HERE ***
        self.particles = []
        num_positions = len(self.legal_positions)

        for i in range(self.num_particles):
            self.particles.append(self.legal_positions[i % num_positions])
    ...
    def observe(self, observation, game_state):
        ...
        if noisy_distance is None: #jailed pacman
            self.particles = [self.get_jail_position()] * self.num_particles
            return

        weights = util.Counter()
        for particle in self.particles:
            distance = util.manhattan_distance(particle, pacman_position)
            weights[particle] += emission_model[distance]

        if sum(weights.values()) == 0:
            self.initialize_uniformly(game_state)
        else:
            self.particles = [util.sample(weights) for _ in range(self.num_particles)]
    ...
    def elapse_time(self, game_state):
        ...
        for old in self.particles:
            new_pos_dist = self.get_position_distribution(self.set_ghost_position(game_state, old))
            new.append(util.sample(new_pos_dist))
        self.particles = new
...
class JointParticleFilter:
    ...
    def initialize_particles(self):
        ...
        every_pos = list(itertools.product(self.legal_positions, repeat=self.num_ghosts)) #old way to convoluted
        random.shuffle(every_pos) #much easier
        self.particles = []
        while len(self.particles) < self.num_particles:
            self.particles += every_pos
        ...
        for particle in self.particles:
            ...
            for i in range(self.num_ghosts):
                if noisy_distances[i] is None:
                    ...
                    curr_distance = util.manhattan_distance(particle[i], pacman_position)
                    weight *= emission_models[i][curr_distance]
            weights[particle] += weight

        if sum(weights.values()) == 0:
            self.initialize_particles()
            for i, distance in enumerate(noisy_distances):
                if distance is None:
                    self.particles = [self.get_particle_with_ghost_in_jail(p, i) for p in self.particles]
        else:
            self.particles = [util.sample(weights) for _ in range(self.num_particles)]
    ...
    def elapse_time(self, game_state):
        ...
        for old_particle in self.particles:
            ...
            # *** YOUR CODE HERE ***
            for i in range(self.num_ghosts):
                prev_ghost_pos = tuple(new_particle)
                new_pos_dist = get_position_distribution_for_ghost(set_ghost_positions(game_state, prev_ghost_pos), i, self.ghost_agents[i])
    ...
    def get_belief_distribution(self):
        ...
        belief_distribution = Counter(self.particles)
        total_count = sum(belief_distribution.values())

        for particle in belief_distribution:
            belief_distribution[particle] /= total_count
