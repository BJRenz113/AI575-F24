...
class GreedyBustersAgent(BustersAgent):
    ...
    def choose_action(self, game_state):
        ...
        curr_most_likely= [] #most likely curr ghost positions
        for ghost_distribution in living_ghost_position_distributions:
            curr_most_likely.append(ghost_distribution.arg_max())

        min_distance = float('inf')   #seeking closest ghost so we can move towards it
        for ghost_pos in curr_most_likely:
            curr_distance = self.distancer.get_distance(pacman_position, ghost_pos)
            if curr_distance < min_distance:
                min_distance = curr_distance
                closest_ghost_pos = ghost_pos

        #seeking best move to closer ghost
        best_action = None
        min_action_distance = float('inf')
        for action in actions:
            next_position = Actions.get_successor(pacman_position, action)
            action_distance = self.distancer.get_distance(next_position, closest_ghost_pos)
            if action_distance < min_action_distance:
                min_action_distance = action_distance
                best_action = action
        return best_action
