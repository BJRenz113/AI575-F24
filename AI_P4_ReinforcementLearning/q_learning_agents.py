...
class QLearningAgent(ReinforcementAgent):
    ...
    def compute_action_from_q_values(self, state):
        ...
        return random.choice(current_best)
    ...
    def update(self, state, action, next_state, reward):
        ...
        # *** YOUR CODE HERE ***
        item = reward + self.discount * self.compute_value_from_q_values(next_state)
        self.q_values[(state, action)] = (1 - self.alpha) * self.get_q_value(state, action) + self.alpha * item
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def get_q_value(self, state, action):
        ...
        # *** YOUR CODE HERE ***
        features_list = self.feat_extractor.get_features(state, action)
        q_value = sum(self.weights[feature] * value for feature, value in features_list.items())
        return q_value
    def update(self, state, action, next_state, reward):
        """
        ...
        # *** YOUR CODE HERE ***
        currQ_val = self.get_q_value(state, action)
        nextMaxQ = self.compute_value_from_q_values(next_state)
        difference = (reward + self.discount * nextMaxQ) - currQ_val
